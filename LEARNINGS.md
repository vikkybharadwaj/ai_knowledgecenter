...
2026-09-26  Landing a *question set* (Vik's 10 eval questions) works best as ONE atomic note per question plus a question-indexed table on the map — it keeps notes atomic and gives a "come back by question" entry point. Also: the same word can mean different axes across sources ("dimensions" = input variation in Hamel's synthetic-data method, = mistake types in the Tide failure map). Call the clash out in both notes rather than silently picking one. And when an official Anthropic doc disagrees with practitioners (volume-over-quality; ROUGE/cosine as graders), record the tension with a reconciliation marked as ours. [→ knowledge/maps/evals.md]
