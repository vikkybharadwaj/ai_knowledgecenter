...
- [An offline eval is a controlled experiment — the agent is real, the world is frozen](../notes/offline-vs-online-evals.md) — how to *measure* reliability: offline (frozen fixtures, known answers) + online (production traffic, rubric judge only) + the flywheel between them; freeze *every* observable input, clock included (lived: Tide's 95 nights of fake 0%).
  - 🧪 **The evals track** grew into its own section, the **[Evals map](evals.md)**. It has ten more foundations lessons: [evals test judgment, unit tests test math](../notes/evals-test-judgment.md) · [freeze everything the model sees](../notes/freeze-everything-the-model-sees.md) · [don't copy, derive](../notes/dont-copy-derive.md) · [a crashed test isn't a failed test](../notes/crashed-is-not-failed.md) · [one run is a sample](../notes/one-run-is-a-sample.md) · [word checks can't read meaning](../notes/word-checks-cant-read-meaning.md) · [a safety alarm is only as good as its checks](../notes/safety-alarm-false-alarms.md) · [dimensions vs slices](../notes/dimensions-vs-slices.md) · [sort before you count](../notes/sort-before-you-count.md) · [whose fault is the failure](../notes/whose-fault-is-the-failure.md).
  - 🧭 **Your 10 eval questions** — the mental model, distilled from Hamel Husain & Shreya Shankar's evals FAQ and Lenny's Newsletter pieces: [what is an eval](../notes/what-is-an-eval.md) · [two kinds of check](../notes/two-kinds-of-eval-check.md) · [synthetic scenarios before launch](../notes/synthetic-scenarios-before-launch.md) · [error analysis on traces](../notes/error-analysis-on-traces.md) · [the production-to-eval flywheel](../notes/production-to-eval-flywheel.md) · [humans and automation](../notes/humans-and-automation-in-evals.md) · [code-check best practices](../notes/code-check-best-practices.md). Error analysis is now its own concept node.
...
- [Evaluate by system shape — one call, RAG, or agent](../notes/evals-by-system-type.md) · [its metrics](../notes/eval-metrics-by-system-type.md) · [aligning an LLM judge](../notes/aligning-an-llm-judge.md) — the pattern-level half of the 10 eval questions: split the eval along the system's stages, pick the numbers per shape, and certify every judge with TPR/TNR.
