...
- [Evaluate by system shape — one call, RAG, or agent](../notes/evals-by-system-type.md) · [its metrics](../notes/eval-metrics-by-system-type.md) · [aligning an LLM judge](../notes/aligning-an-llm-judge.md) — the pattern-level half of the ten mental models: split the eval along the system's stages, pick the numbers per shape, and certify every judge with TPR/TNR.
